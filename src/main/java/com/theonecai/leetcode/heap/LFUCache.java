 ...
 * leetcode 460
 * @Author: theonecai
 * @Date: Create in 2020/6/15 16:26
 * @Description:
 */
class LFUCache {
    ...
    Map<Integer, Integer> indexCache;
    ...
    public LFUCache(int capacity) {
        ...
        indexCache = new HashMap<>(capacity);
    }
...
//            node.atime = System.nanoTime();
        ...
        if (size + 1 > capacity) {
            indexCache.remove(minHeap[1].key);
            minHeap[1] = node;
            fixDown(1);
            return;
        }
    ...
    private int getIndex(int key) {
        Integer idx = indexCache.get(key);
    ...
    private int compare(Node node, Node node2) {
        if (node.count != node2.count) {
            return node.count - node2.count;
        }
        return node.atime >= node2.atime ? 1 : -1;
    }
    ...
    private int fixUp(int index) {
        int upIndex = index/2;
        int current = index;
        indexCache.put(minHeap[current].key, current);
        while (upIndex > 0) {
            if (compare(minHeap[current], minHeap[upIndex]) < 0) {
    ...
    private void swap(int from, int to) {
        Node tmp = minHeap[from];
        minHeap[from] = minHeap[to];
        minHeap[to] = tmp;

        indexCache.put(minHeap[from].key, from);
        indexCache.put(minHeap[to].key, to);
    }
    ...
    private int fixDown(int index) {
        ...
        while (downIndex <= size) {
            ...
            if (rightIdx <=size && compare(minHeap[leftIdx], minHeap[rightIdx]) > 0) {
                downIndex = rightIdx;
            }
            if (compare(minHeap[current], minHeap[downIndex]) > 0) {
...
//            System.out.print("[key="+ minHeap[i].key + ",value=" + minHeap[i].value + ",count=" + minHeap[i].count + ",atime=" + minHeap[i].atime + "] ");
            System.out.print("[" + minHeap[i].key + ":" + minHeap[i].value + "," + minHeap[i].count + "," + minHeap[i].atime + "],");
        }
        System.out.println("");
    }
